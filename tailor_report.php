...
// --- 1. Fetch Summary Statistics (These remain unfiltered) ---

$total_tailors = $db->selectData("SELECT COUNT(tid) as count FROM treg")->fetch_assoc()['count'];
$approved_tailors = $db->selectData("SELECT COUNT(tid) as count FROM treg WHERE status = 'Approved'")->fetch_assoc()['count'];
$pending_tailors = $db->selectData("SELECT COUNT(tid) as count FROM treg WHERE status = 'Pending'")->fetch_assoc()['count'];
$removed_tailors = $db->selectData("SELECT COUNT(tid) as count FROM treg WHERE status = 'Removed'")->fetch_assoc()['count'];
$total_assigned_requests = $db->selectData("SELECT COUNT(sdid) as count FROM stitchreq WHERE tid IS NOT NULL AND tid > 0")->fetch_assoc()['count'];
$total_designs_uploaded = $db->selectData("SELECT COUNT(did) as count FROM upload")->fetch_assoc()['count'];

// --- 2. Fetch Detailed Report Data with Filtering ---

// Get the current filter status from the URL, if any
...
$sql_detailed = "
    SELECT 
        ...
        t.email,
        t.status,
        (SELECT COUNT(sdid) FROM stitchreq WHERE tid = t.tid) as assigned_requests,
        (SELECT COUNT(did) FROM upload WHERE uid = t.tid) as designs_uploaded
    FROM 
        treg t
";
...
if ($filter_status !== 'all' && in_array($filter_status, ['Approved', 'Pending', 'Removed'])) {
    $sql_detailed .= " WHERE t.status = ?";
    $types = "s";
    $params = [$filter_status];
}

$sql_detailed .= " ORDER BY assigned_requests DESC, t.tname ASC";
...
<head>
    ...
    <title>Tailor Report - StitchVerse Admin</title>
...
<body class="bg-gray-100">
          ...
          <div class="relative">
            ...
            <div class="relative">
                ...
                <div id="report-dropdown-menu" class="hidden absolute mt-2 w-48 bg-white rounded-lg shadow-xl py-2 z-20 border border-gray-100">
                    <a href="tailor_report.php" class="block px-4 py-2 text-sm text-purple-600 bg-purple-50 font-semibold">Tailor Reports</a>
    ...
    <main class="container mx-auto px-6 py-12">
        <div class="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8 no-print">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Tailor Performance Report</h1>
                <p class="text-gray-500 mt-1">An overview of tailor registrations, activities, and contributions.</p>
        ...
        <!-- Summary Section - Now clickable filters -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12 no-print">
            <a href="tailor_report.php" class="block bg-white p-6 rounded-2xl shadow-lg border border-gray-200 flex items-center gap-5 hover:border-blue-500 hover:shadow-md transition-all">
                <div class="w-16 h-16 bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center shrink-0"><i class="ri-group-fill text-3xl"></i></div>
                <div><p class="text-gray-500 text-sm font-medium">Total Tailors</p><p class="text-3xl font-bold text-gray-800"><?php echo $total_tailors; ?></p></div>
            </a>
            <a href="tailor_report.php?status=Approved" class="block bg-white p-6 rounded-2xl shadow-lg border border-gray-200 flex items-center gap-5 hover:border-green-500 hover:shadow-md transition-all">
                <div class="w-16 h-16 bg-green-100 text-green-600 rounded-2xl flex items-center justify-center shrink-0"><i class="ri-user-follow-fill text-3xl"></i></div>
                <div><p class="text-gray-500 text-sm font-medium">Approved</p><p class="text-3xl font-bold text-gray-800"><?php echo $approved_tailors; ?></p></div>
            </a>
            <a href="tailor_report.php?status=Pending" class="block bg-white p-6 rounded-2xl shadow-lg border border-gray-200 flex items-center gap-5 hover:border-yellow-500 hover:shadow-md transition-all">
                <div class="w-16 h-16 bg-yellow-100 text-yellow-600 rounded-2xl flex items-center justify-center shrink-0"><i class="ri-time-fill text-3xl"></i></div>
                <div><p class="text-gray-500 text-sm font-medium">Pending Approval</p><p class="text-3xl font-bold text-gray-800"><?php echo $pending_tailors; ?></p></div>
            </a>
            <a href="tailor_report.php?status=Removed" class="block bg-white p-6 rounded-2xl shadow-lg border border-gray-200 flex items-center gap-5 hover:border-red-500 hover:shadow-md transition-all">
                ...
                <div><p class="text-gray-500 text-sm font-medium">Removed / Rejected</p><p class="text-3xl font-bold text-gray-800"><?php echo $removed_tailors; ?></p></div>
            </a>
            <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 flex items-center gap-5">
                ...
                <div><p class="text-gray-500 text-sm font-medium">Assigned Requests</p><p class="text-3xl font-bold text-gray-800"><?php echo $total_assigned_requests; ?></p></div>
            </div>
             <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 flex items-center gap-5">
                <div class="w-16 h-16 bg-pink-100 text-pink-600 rounded-2xl flex items-center justify-center shrink-0"><i class="ri-palette-fill text-3xl"></i></div>
                <div><p class="text-gray-500 text-sm font-medium">Designs Uploaded</p><p class="text-3xl font-bold text-gray-800"><?php echo $total_designs_uploaded; ?></p></div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden printable-area">
            <div class="p-6 border-b border-gray-200 flex flex-col md:flex-row justify-between md:items-center gap-4">
                <div>
                    <h2 class="text-xl font-bold text-gray-800">Detailed Tailor Breakdown</h2>
                ...
                <?php if ($filter_status !== 'all'): ?>
                    <a href="tailor_report.php" class="no-print text-sm font-semibold text-gray-600 hover:text-purple-600 flex items-center gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-600">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                       <tr>
                            <th scope="col" class="px-6 py-3">Tailor Name</th>
                            <th scope="col" class="px-6 py-3">Email</th>
                            <th scope="col" class="px-6 py-3 text-center">Status</th>
                            <th scope="col" class="px-6 py-3 text-center">Stitch Requests</th>
                            <th scope="col" class="px-6 py-3 text-center">Designs Uploaded</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if (empty($tailor_details)): ?>
                            <tr><td colspan="5" class="px-6 py-16 text-center"><i class="ri-user-search-line text-5xl text-gray-300"></i><h3 class="text-xl font-semibold text-gray-800 mt-4">No Tailors Found</h3><p class="text-gray-500 mt-1">No tailors match the current filter.</p></td></tr>
                        <?php else: ?>
                            <?php foreach ($tailor_details as $tailor): ?>
                            <tr class="bg-white border-b hover:bg-gray-50">
                                <td class="px-6 py-4 font-bold text-gray-900"><?php echo htmlspecialchars($tailor['tname']); ?></td>
                                <td class="px-6 py-4 text-gray-500"><?php echo htmlspecialchars($tailor['email']); ?></td>
                                <td class="px-6 py-4 text-center">
                                    <span class="px-2 py-1 text-xs font-semibold rounded-full <?php echo getStatusClass($tailor['status']); ?>">
                                        <?php echo htmlspecialchars($tailor['status']); ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-center font-medium text-gray-800"><?php echo $tailor['assigned_requests']; ?></td>
                                <td class="px-6 py-4 text-center font-medium text-gray-800"><?php echo $tailor['designs_uploaded']; ?></td>
                            </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
