...
$sql = "SELECT
            p.order_id, p.pdate,
            u.dname, u.dtype, u.dprice, u.dimg,
        ...
        INNER JOIN treg AS t ON u.uid = t.tid
...
if ($result && $result->num_rows === 1) {
    ...
    echo "<script>alert('Receipt not found or you do not have permission to view it.'); window.location='designorders.php';</script>";
    exit();
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Invoice #INV-<?php echo htmlspecialchars($receipt['order_id']); ?> - StitchVerse</title>
...
<body class="bg-gray-100">
    ...
    <div class="no-print container mx-auto px-6 py-4 flex items-center justify-between">
        <a href="designorder_details.php?id=<?php echo htmlspecialchars($receipt['order_id']); ?>" class="text-sm font-medium text-purple-600 hover:text-purple-800 flex items-center gap-2">
            <i class="ri-arrow-left-line"></i> Back to Order Details
        </a>
        ...
        <div class="max-w-4xl mx-auto bg-white p-8 sm:p-12 rounded-xl shadow-lg">
            <div class="flex justify-between items-start border-b border-gray-200 pb-6">
                ...
                <div class="text-right">
                    ...
                    <p class="text-sm text-gray-500">#INV-<?php echo htmlspecialchars($receipt['order_id']); ?></p>
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-8 mt-8">
                ...
                <div class="md:text-right">
                    ...
                    <p class="text-gray-600 text-sm"><span class="font-semibold text-gray-800">Order Date:</span> <?php echo date("d M, Y", strtotime($receipt['orderdate'])); ?></p>
                </div>
            </div>
                        ...
                        <tr>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-4">
                                    <img src="<?php echo htmlspecialchars($receipt['dimg']); ?>" class="w-12 h-16 object-cover rounded-md border">
                                    <div>
                                        <p class="font-semibold text-gray-800"><?php echo htmlspecialchars($receipt['dname']); ?></p>
                                        <p class="text-sm text-gray-600"><?php echo htmlspecialchars($receipt['dtype']); ?> by <?php echo htmlspecialchars($receipt['tname']); ?></p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-right font-medium text-gray-800">₹<?php echo number_format($receipt['dprice'], 2); ?></td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="mt-8 flex justify-end">
                <div class="w-full max-w-xs space-y-3">
                    <div class="flex justify-between text-gray-600"><span>Subtotal</span><span>₹<?php echo number_format($receipt['dprice'], 2); ?></span></div>
                    <div class="flex justify-between text-gray-600"><span>Delivery Fee</span><span>₹40.00</span></div>
                    <div class="flex justify-between text-lg font-bold text-gray-800 border-t pt-3 mt-2"><span>Total Paid</span><span class="text-purple-600">₹<?php echo number_format($receipt['dprice'] + 40, 2); ?></span></div>
                </div>
            </div>
