...
// --- Data Fetching Logic ---
// We query the database to get ONLY tailors with the status 'Approved'.
// This is more efficient than fetching all tailors and filtering in PHP.
$approved_tailors = [];
$sql = "SELECT tid, tname, email, city, distri, phone, spect FROM treg WHERE status = 'Approved' ORDER BY tname ASC";
$result = $db->selectData($sql);

if ($result) {
    while ($row = $result->fetch_assoc()) {
        $approved_tailors[] = $row;
    }
}
// --- Prepare Flash Messages for JavaScript ---
// Check for session messages and prepare them for the script. Then, unset them immediately.
...
<head>
    ...
    <title>Approved Tailors - StitchVerse Admin</title>
...
<body class="bg-gray-100">
    ...
    <main class="container mx-auto px-6 py-12">
        <div class="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Approved Tailors</h1>
                <p class="text-gray-500 mt-1">A list of all active and approved tailors on the platform.</p>
            </div>
            <div class="bg-gradient-to-br from-green-100 to-green-200 p-4 rounded-xl text-center shrink-0">
                <div class="text-3xl font-bold text-green-700"><?php echo count($approved_tailors); ?></div>
                <div class="text-green-700 text-sm font-medium">Active Tailors</div>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-600">
                   <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                    ...
                    <tbody>
                        <?php if (empty($approved_tailors)): ?>
                            <tr><td colspan="5" class="px-6 py-10 text-center text-gray-500">No approved tailors found.</td></tr>
                        <?php else: ?>
                            <?php foreach ($approved_tailors as $tailor): ?>
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4"><?php echo htmlspecialchars($tailor['spect']); ?></td>
                                <td class="px-6 py-4 text-center">
                                    <a href="viewapp_tailors.php?id=<?php echo $tailor['tid']; ?>" class="font-medium text-white bg-purple-600 hover:bg-purple-700 px-4 py-2 rounded-lg text-xs inline-flex items-center gap-1">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (menuButton && mobileMenu) {
                // You can populate the mobile menu dynamically or just copy it from the other page
                // For simplicity, we'll assume the user navigates back to the homepage for the mobile menu
            }
            ...
            if (successMessage) {
                ...
                setTimeout(() => popup.classList.remove('translate-x-full'), 50); // Slide in

                setTimeout(() => {
                    popup.classList.add('translate-x-full'); // Slide out
