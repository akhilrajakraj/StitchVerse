...
// --- Data Fetching Logic ---
// This query now finds all stitch requests that have been accepted by the tailor.
// It also fetches the price to differentiate between requests awaiting a quote and those awaiting payment.
$accepted_requests = [];
$sql = "SELECT
            sr.sdid,
            sr.sddate,
            ...
            sr.sdtype,
            sr.sstatus,
            sr.sprice, -- Added to check if a quote has been given
            c.cname,
            c.phone
        FROM
            stitchreq sr
        ...
        WHERE
            sr.tid = ? AND (sr.sstatus = 'Accepted' OR sr.sstatus = 'Paid')
        ORDER BY
            sr.sddate ASC";
...
<head>
    ...
    <title>Active Jobs - StitchVerse</title>
...
<body class="bg-gradient-to-br from-purple-50 to-pink-50 min-h-screen">
    ...
    <!-- === HEADER: WITH ACTIVE STATE FOR 'ACCEPTED REQUESTS' === -->
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <div class="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center mr-4">
                                <i class="ri-scissors-cut-line text-2xl text-white"></i>
                            </div>
                            <div>
                                <h1 class="text-3xl font-bold text-white">Active Jobs</h1>
                                <p class="text-purple-100 mt-1">All jobs you've accepted, including those awaiting a price quote.</p>
                            </div>
                        </div>
                    </div>
                </div>
                       ...
                       <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                        ...
                        <tbody>
                            <?php if (empty($accepted_requests)): ?>
                                <tr><td colspan="7" class="px-6 py-10 text-center text-gray-500">You have no active jobs at this time.</td></tr>
                            <?php else: ?>
                                ...
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">
                                        <!-- MODIFICATION START: Added logic to show different status based on price -->
                                        <?php if ($request['sstatus'] == 'Paid'): ?>
                                        ...
                                        <?php elseif ($request['sstatus'] == 'Accepted'): ?>
                                            <?php if (isset($request['sprice']) && $request['sprice'] > 0): ?>
                                                <span class="bg-yellow-100 text-yellow-800 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded-full">
                                                    <i class="ri-time-line mr-1"></i>
                                                    Awaiting Payment
                                                </span>
                                            <?php else: ?>
                                                <span class="bg-blue-100 text-blue-800 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded-full">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <a href="viewacceptedreqdetails.php?id=<?php echo $request['sdid']; ?>" class="font-medium text-purple-600 hover:text-purple-800 text-xs inline-flex items-center gap-1">
