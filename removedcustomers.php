...
// --- Data Fetching Logic ---
// We find removed customers by looking for the 'removed_customer' utype in the login table
// and then joining with the creg table to get their full details.
$removed_customers = [];
$sql = "SELECT c.cid, c.cname, c.email, c.phone, c.city, c.distr 
        FROM creg c 
        JOIN login l ON c.cid = l.uid 
        WHERE l.utype = 'removed_customer' 
        ORDER BY c.cname ASC";
$result = $db->selectData($sql);

if ($result) {
    while ($row = $result->fetch_assoc()) {
        $removed_customers[] = $row;
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Removed Customers - StitchVerse Admin</title>
...
<body class="bg-gray-100">
    ...
    <main class="container mx-auto px-6 py-12">
        ...
        <div class="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Removed Customers</h1>
                <p class="text-gray-500 mt-1">A historical log of all customer accounts removed by an administrator.</p>
            </div>
            <div class="bg-gradient-to-br from-gray-200 to-gray-300 p-4 rounded-xl text-center shrink-0">
                <div class="text-3xl font-bold text-gray-700"><?php echo count($removed_customers); ?></div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-600">
                   <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                    ...
                    <tbody>
                        <?php if (empty($removed_customers)): ?>
                            <tr><td colspan="4" class="px-6 py-10 text-center text-gray-500">No removed customer accounts found.</td></tr>
                        <?php else: ?>
                            <?php foreach ($removed_customers as $customer): ?>
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="viewremovedcustomer.php?id=<?php echo $customer['cid']; ?>" class="font-medium text-purple-600 hover:text-purple-800 text-xs inline-flex items-center gap-1">
