...
// --- Data Fetching Logic ---
// This query finds all stitch requests that have been cancelled by the logged-in tailor.
$cancelled_requests = [];
$sql = "SELECT
            sr.sdid,
            sr.sddate,
        ...
        WHERE
            sr.tid = ? AND sr.sstatus = 'Cancelled'
        ORDER BY
            sr.sddate DESC";
...
<head>
    ...
    <title>Cancelled Requests - StitchVerse</title>
...
<body class="bg-gradient-to-br from-purple-50 to-pink-50 min-h-screen">
    ...
    <!-- === HEADER: WITH ACTIVE STATE FOR 'CANCELLED REQUESTS' === -->
                ...
                <div class="bg-gradient-to-r from-purple-600 to-pink-600 px-8 py-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div>
                                <h1 class="text-3xl font-bold text-white">Cancelled Requests</h1>
                                <p class="text-purple-100 mt-1">A history of job requests that you have cancelled.</p>
                            </div>
                        </div>
                    </div>
                </div>
                       ...
                       <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                        ...
                        <tbody>
                            <?php if (empty($cancelled_requests)): ?>
                                <tr><td colspan="7" class="px-6 py-10 text-center text-gray-500">You have not cancelled any requests.</td></tr>
                            <?php else: ?>
                                ...
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 font-medium text-gray-500"><?php echo date("M d, Y", strtotime($request['sddate'])); ?></td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="bg-gray-200 text-gray-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full">
                                            <?php echo htmlspecialchars($request['sstatus']); ?>
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <a href="viewcancelledreqdetails.php?id=<?php echo $request['sdid']; ?>" class="font-medium text-purple-600 hover:text-purple-800 text-xs inline-flex items-center gap-1">
